// zkME Reputation Protocol Contract - Converted from Solidity to Rust (Anchor Framework)
...
// Declare Program ID
declare_id!("ZkMeReputation111111111111111111111111111111");

#[program]
pub mod zkme_reputation {
    use super::*;

    pub fn initialize(ctx: Context<Initialize>, token_mint: Pubkey) -> Result<()> {
        let settings = &mut ctx.accounts.settings;
        settings.owner = *ctx.accounts.owner.key;
        settings.zkme_token_mint = token_mint;
        settings.zkme_per_question = 1_000_000_000_000_000_000; // 1e18
        settings.min_cap = 5;
    ...
    pub fn add_warranty(ctx: Context<AddWarranty>, amount: u64) -> Result<()> {
        ...
        ctx.accounts.statistics.total_zkme_warranty += amount;
        Ok(())
    }
...
pub struct Settings {
    pub owner: Pubkey,
    pub zkme_token_mint: Pubkey,
    pub max_cap: u64,
    pub min_cap: u64,
    pub zkme_per_question: u64,
    pub service_fee: u8,
}
...
pub struct Statistics {
    pub total_zkme_burnt: u64,
    pub total_zkme_fee: u64,
    pub total_zkme_warranty: u64,
}
